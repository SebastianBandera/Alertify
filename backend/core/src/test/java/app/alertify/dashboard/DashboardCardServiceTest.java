...
class DashboardCardServiceTest {
    ...
    void pagesAlertsByIdAndAssemblesTilesFromBatchQueries() {
        ...
        AlertExecution previousIssue = AlertExecution.error(UUID.randomUUID(), ran, null, finishedAt.minusSeconds(600), finishedAt.minusSeconds(599), finishedAt.minusSeconds(598), "java.io.IOException", "Connection refused", null);
        ReflectionTestUtils.setField(previousIssue, "id", 39L);
        ...
        when(executionQuery.previousIssueExecutionIds(eq(Map.of(5L, 40L)), any(Instant.class))).thenReturn(Map.of(5L, 39L));
        when(executionRepository.findAllById(anyCollection())).thenReturn(List.of(execution, previousIssue));
