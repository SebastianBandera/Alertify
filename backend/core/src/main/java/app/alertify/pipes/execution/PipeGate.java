...
/** Coordinates active executions and FIFO Hook waiters for one Pipe. */
final class PipeGate {
    private int active;
        ...
        synchronized (this) {
            ...
            try {
                while (!waiter.assigned && remaining > 0) {
                    wait(Math.clamp(Duration.ofNanos(remaining).toMillis(), 1, Integer.MAX_VALUE));
