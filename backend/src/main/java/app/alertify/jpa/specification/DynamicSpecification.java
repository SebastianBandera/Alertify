...
public final class DynamicSpecification {
    ...
    public static <T> Specification<T> from(
        ...
        return (root, query, cb) -> {
            ...
            params.forEach((field, values) -> {
                if (RESERVED_PARAMS.contains(field) || values == null || values.isEmpty()) return;
    ...
    private static Predicate createPredicate(
            CriteriaBuilder cb, Path<?> path, Class<?> type, String rawValue) {
        String value = rawValue.trim();

        if ("=null".equalsIgnoreCase(value)) return cb.isNull(path);
        if ("!=null".equalsIgnoreCase(value)) return cb.isNotNull(path);
        if (type == String.class) return createStringPredicate(cb, path, value);
        if (isRange(value)) return createRangePredicate(cb, path, type, value);
        if (value.startsWith("!=")) return cb.notEqual(path, convert(value.substring(2), type));
        if (value.startsWith(">=")) return greaterThanOrEqualTo(cb, path, convertComparable(value.substring(2), type));
        if (value.startsWith("<=")) return lessThanOrEqualTo(cb, path, convertComparable(value.substring(2), type));
        if (value.startsWith(">")) return greaterThan(cb, path, convertComparable(value.substring(1), type));
        if (value.startsWith("<")) return lessThan(cb, path, convertComparable(value.substring(1), type));
    ...
    private static Predicate createStringPredicate(CriteriaBuilder cb, Path<?> path, String value) {
        ...
        if (flexible) value = value.substring(1);

        boolean distinct = value.startsWith("!=");
        if (distinct) value = value.substring(2);
    ...
    private static Predicate createRangePredicate(
            CriteriaBuilder cb, Path<?> path, Class<?> type, String value) {
        int separator = value.indexOf('_');
