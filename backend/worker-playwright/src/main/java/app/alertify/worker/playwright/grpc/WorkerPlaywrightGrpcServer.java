...
/**
 * Owns the Playwright worker's native Netty gRPC server, requires a backend
 * certificate through mTLS and publishes its cumulative capabilities.
 */
@Component
public class WorkerPlaywrightGrpcServer implements SmartLifecycle {
    ...
    private io.grpc.ServerCredentials serverCredentials() {
        WorkerPlaywrightGrpcServerProperties.Tls tls = properties.tls();
        if (tls == null || !tls.enabled())
            return io.grpc.InsecureServerCredentials.create();

        requireReadableFile(tls.certificateChain(), "worker-playwright.grpc.tls.certificate-chain");
        requireReadableFile(tls.privateKey(), "worker-playwright.grpc.tls.private-key");
        requireReadableFile(tls.clientCaCertificate(), "worker-playwright.grpc.tls.client-ca-certificate");
        try {
            ...
            throw new IllegalStateException("The Playwright worker gRPC mTLS credentials could not be loaded", exception);
        }
    }
