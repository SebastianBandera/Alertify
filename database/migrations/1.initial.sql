...
-- secrets), kept in a table of its own instead of alongside user-editable
-- configurations. There is no value_type column: the jsonb value already
-- carries its own shape, and every entry here is treated as sensitive by
-- convention (masked in the UI, value only replaceable, never displayed).
...
-- The value is deliberately not audited: it is sensitive key material and
-- must never end up in the audit history, even encrypted at rest.
