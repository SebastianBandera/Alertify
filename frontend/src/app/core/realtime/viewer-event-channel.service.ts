...
export type ViewerEventConnectionState = 'idle' | 'connecting' | 'connected' | 'reconnecting';

interface ViewerEventPayloads {
  readonly DASHBOARD_ALERT: DashboardAlertCard;
  readonly DASHBOARD_ALERT_REMOVED: { readonly alertId: number };
}

export type ViewerEventName = keyof ViewerEventPayloads;

const EVENT_NAMES: ReadonlySet<string> = new Set<ViewerEventName>(['DASHBOARD_ALERT', 'DASHBOARD_ALERT_REMOVED']);

function isEventName(value: unknown): value is ViewerEventName {
  return typeof value === 'string' && EVENT_NAMES.has(value);
}

type ViewerEventMessage = {
  [Name in ViewerEventName]: {
    readonly name: Name;
    readonly payload: ViewerEventPayloads[Name];
  }
}[ViewerEventName];
...
/** Dashboard-only real-time channel for non-administrative viewers. */
@Injectable({ providedIn: 'root' })
export class ViewerEventChannelService {
  private readonly authService = inject(AuthService);
  private readonly document = inject(DOCUMENT);
  private readonly destroyRef = inject(DestroyRef);
  private readonly apiBaseUrl = inject(RUNTIME_CONFIG).apiBaseUrl;
  private readonly events = new Subject<ViewerEventMessage>();
  private readonly pendingRequests = new Map<string, PendingRequest>();
  readonly connectionState = signal<ViewerEventConnectionState>('idle');
  ...
  stop(): void {
    ...
    this.rejectPendingRequests('The viewer event channel was closed.');
    this.connectionState.set('idle');
  }

  on<Name extends ViewerEventName>(name: Name): Observable<ViewerEventPayloads[Name]> {
    return this.events.pipe(
      filter((message) => message.name === name),
      map((message) => message.payload as ViewerEventPayloads[Name]),
    );
  }
  ...
  request<Response>(name: string, payload: unknown): Promise<Response> {
    ...
    if (!window || !socket || socket.readyState !== WebSocket.OPEN || this.connectionState() !== 'connected') {
      return Promise.reject(new Error('The viewer event channel is not connected.'));
    }
    ...
    return new Promise<Response>((resolve, reject) => {
      const timeout = window.setTimeout(() => {
        ...
        reject(new Error('The viewer request timed out.'));
      ...
      try {
        ...
        this.pendingRequests.delete(requestId);
        reject(error instanceof Error ? error : new Error('The viewer request could not be sent.'));
      }
    });
  }
  ...
  private connect(): void {
    ...
    const socketUrl = new URL(`${this.apiBaseUrl}/api/viewer/events`);
  ...
  private handleEvent(message: Record<string, unknown>): void {
    ...
    this.events.next({ name, payload: message['payload'] } as ViewerEventMessage);
  }
  ...
  private handleSocketClosed(socket: WebSocket): void {
    ...
    this.socket = null;
    this.clearTokenRenewalTimer();
    this.rejectPendingRequests('The viewer event channel was disconnected.');
    if (this.stopped) return;
